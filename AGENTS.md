...
- `POST /api/ai/review-brief`（SSE）：入参 `{ brief, draft, model?, temperature? }`，以 `event: delta` 推送正文。prompt（`server-review.ts`）采用「先抽规则、再分维核对」：先把 Brief 硬性要求抽成带编号/类型/等级的 rubric，再分缺失检查（must）与违规检查（forbid）两轮。正文用独立成行的块标记：`<<<REQUIREMENTS>>>`（`- R01 [must|high] 要求` 规则清单）、`<<<OVERALL>>>`（结论：通过/不通过 + 说明）、`<<<MISSING>>>`（规则:Rxx/等级:high|medium|low/要求/缺失/建议，可多个）、`<<<VIOLATION>>>`（规则/等级/要求/草稿/quote/建议，可多个，quote 为草稿原文片段用于定位）、`<<<SUGGESTION>>>`（软建议 `- ` 列表，可省略）。`useBriefReview` 边收边解析为 `{rules, verdict, summary, missing, violations, suggestions}`，问题按等级（high>medium>low）再按规则号排序；结论与清单冲突时以清单为准。设计参考 brand-compliance-checker（rubric 编号化+机器可读契约）、电商三级审核引擎（风险账本四要素）、BrandSense（缺失/违规双检查器）。只依据 Brief 自身硬性要求，不叠加违禁词库；不保存审核结果历史，但左右两侧文本通过 localStorage 留存（`review-brief:v1` / `review-draft:v1`，见 storage 的 loadReviewDraft/saveReviewDraft），刷新或再次打开自动恢复，「清空」清除。
